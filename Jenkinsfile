...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="suhodo/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/SJF02/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.login=c8ba875135ebabd22487626c44c72f2f47e90e9a
                    '''
                }
                */
            }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.0.110 sudo docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.0.110 sudo docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
